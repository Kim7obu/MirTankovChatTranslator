...
MOD_VERSION = '1.5.0'
UI_LANGUAGE = 'ko'
LINKAGE = 'Tonymocchi.chattranslator.ko'
SETTINGS_VERSION = 2
...
# ---------------------------------------------------------------------------
# embedded preserve dictionary
#
# The JSON is packaged inside the MTMOD. It is parsed once during init and is
# then represented by a frozenset (whole-message O(1) checks) plus one compiled
# longest-first regex (terms embedded in a sentence). There is no file access,
# JSON parsing or per-entry loop on the chat hot path.
#
# A registered term is never translated. A line consisting only of such a
# term passes through untouched. Inside a longer Russian sentence, registered
# terms are temporarily masked before the HTTP request and restored verbatim
...
def _parse_preserve_dictionary(raw):
    ...
    terms = raw.get('preserveTerms')
    if not isinstance(terms, list):
        raise ValueError('preserveTerms must be a JSON array')
    if len(terms) > PRESERVE_DICTIONARY_MAX_TERMS:
        raise ValueError('too many preserveTerms entries')
    ...
    exact = frozenset(unique.keys())
    ordered_terms = sorted(unique.values(), key=lambda item: len(item), reverse=True)
    pattern = None
    if enabled and ordered_terms:
        alternatives = [_term_regex(term, normalize_yo) for term in ordered_terms]
        body = u'(?:%s)' % u'|'.join(alternatives)
        if whole_words:
            body = (u'(?<![%s])%s(?![%s])'
                    % (_DICTIONARY_WORD_CHARS, body, _DICTIONARY_WORD_CHARS))
...
def load_preserve_dictionary(raw_bytes=None):
    ...
    try:
        ...
        _log('preserve dictionary loaded (%d terms, revision=%s)'
             % (len(_dictionary_exact), _dictionary_revision), force=True)
        return len(_dictionary_exact)
    except Exception:
...
def protect_dictionary_terms(text):
    pattern = _dictionary_pattern
    source = _as_unicode(text)
    if not _dictionary_enabled or pattern is None:
        return source, ()
    replacements = []
    ...
    return pattern.sub(replace, source), tuple(replacements)
...
def _cache_prefix():
    return u'v2:%s\t' % _dictionary_revision
...
def translate_blocking(text, target):
    ...
    request_text, replacements = protect_dictionary_terms(text)
...
def _should_translate(message, text):
    ...
    if cfg.get('skipCommands'):
        if message_is_command(message) or looks_like_command(text):
            return False
        ...
        {'type': 'Empty', 'height': 8},
        {
            ...
            'tooltip': _tip(
                ...
                L(u'"рак", "свет", "топлюсь" 처럼 게임에서만 통하는 은어를 즉시, 그리고 '
                  u'기계번역보다 정확하게 옮깁니다. 내장 JSON 보호 사전은 '
                  u'이 설정과 무관하게 적용됩니다.',
                  ...
                  u'embedded JSON preserve dictionary is applied independently.')),
        },
        {
            'type': 'CheckBox',
...
def init():
    ...
    try:
        ...
        if BigWorld is not None:
            ...
            register_settings()
        _log('%s %s ready (%d hook(s), phrasebook %d, preserve terms %d)'
